...
def upgrade() -> None:
    ...
    op.drop_constraint(
        "assessments_skill_dna_id_fkey", "assessments", type_="foreignkey"
    )
    ...
    op.drop_constraint(
        "job_descriptions_user_id_fkey", "job_descriptions", type_="foreignkey"
    )
    op.drop_constraint(
        "capability_heatmaps_user_id_fkey", "capability_heatmaps", type_="foreignkey"
    )
    ...
    op.drop_index(
        op.f("idx_challenges_assessment_id"), table_name="practical_challenges"
    )
    ...
    op.drop_constraint(
        "practical_challenges_assessment_id_fkey",
        "practical_challenges",
        type_="foreignkey",
    )
    op.drop_constraint(
        "evaluations_response_id_fkey", "evaluations", type_="foreignkey"
    )
    op.drop_constraint("evidence_evaluation_id_fkey", "evidence", type_="foreignkey")
    op.drop_constraint(
        "learning_paths_evaluation_id_fkey", "learning_paths", type_="foreignkey"
    )
    op.drop_constraint(
        "career_compasses_cyber_twin_id_fkey", "career_compasses", type_="foreignkey"
    )
    ...
    op.add_column(
        "users", sa.Column("display_name", sa.String(length=255), nullable=False)
    )
    op.alter_column(
        "users",
        "id",
    ...
    op.alter_column(
        ...
        existing_type=postgresql.ENUM(
            "admin", "capability_analyst", "professional", "reviewer", name="user_role"
        ),
    ...
    op.alter_column(
        ...
        "status",
        existing_type=postgresql.ENUM(
            "planned", "active", "completed", "failed", name="assessment_status"
        ),
        type_=sa.String(length=50),
        nullable=True,
    )
    op.add_column(
        "assessments", sa.Column("candidate_id", sa.String(length=12), nullable=False)
    )
    op.add_column(
        "assessments", sa.Column("domain", sa.String(length=255), nullable=False)
    )
    op.add_column(
        "assessments",
    ...
    op.add_column(
        "cyber_twins", sa.Column("overall_confidence", sa.Float(), nullable=True)
    )
