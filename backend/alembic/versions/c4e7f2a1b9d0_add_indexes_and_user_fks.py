...
def upgrade() -> None:
    ...
    op.create_index("ix_question_records_assessment_id", "question_records", ["assessment_id"])
    ...
    op.create_foreign_key("fk_cyber_twins_user_id", "cyber_twins", "users", ["user_id"], ["id"])

    op.add_column("learning_roadmaps", sa.Column("user_id", sa.String(12), nullable=True))
    op.create_index("ix_learning_roadmaps_user_id", "learning_roadmaps", ["user_id"])
    op.create_foreign_key("fk_learning_roadmaps_user_id", "learning_roadmaps", "users", ["user_id"], ["id"])

    op.add_column("answer_repair_guides", sa.Column("user_id", sa.String(12), nullable=True))
    op.create_index("ix_answer_repair_guides_user_id", "answer_repair_guides", ["user_id"])
    op.create_foreign_key("fk_answer_repair_guides_user_id", "answer_repair_guides", "users", ["user_id"], ["id"])
...
def downgrade() -> None:
    ...
    op.drop_foreign_key("fk_answer_repair_guides_user_id", table_name="answer_repair_guides")
