# PWNDORA SkillScan X — Non-Functional Requirements
...
This document defines the quality attributes and operational characteristics that PWNDORA SkillScan X must satisfy. Unlike Functional Requirements, which describe **what** the platform does, Non-Functional Requirements define **how well** it performs.
...
```mermaid
flowchart TD
    C[Correctness] --> S[Security] --> R[Reliability] --> E[Explainability] --> P[Performance] --> M[Maintainability] --> U[Usability]
...
| Career Compass | < 5 seconds | LLM recommendation |
...
| Report + Career Compass | < 8 seconds combined | GPT-4o, structured output |
...
| Data leakage prevention | No PII in AI prompts; anonymize professional responses |
...
| New cyber role (e.g., Cloud Security Engineer) | Skill DNA Profile template | < 2 hours |
...
| Evidence citation | Specific professional statements quoted per score |
...
| NICE NIST SP 800-181 | Role and capability framework | Implemented for assessment |
...
| Voice recognition failure | Medium — professional must switch to text | Medium | Manual text fallback always available; no loss of functionality |
| Session data loss | High — professional must restart | Low | Auto-save after every turn; write-ahead logging |
...
| G-03 | System generates a Capability Blueprint within 15 seconds |
| G-04 | Professional can complete a 5+ turn conversational assessment |
...
| G-08 | Career Compass with 3+ topics is generated |
| G-09 | Full session transcript is viewable with annotations |
| G-10 | Capability analyst report view displays readiness level and capability profile |
...
- [Privacy & Security Model](../docs/concepts/privacy-security-model.md)
